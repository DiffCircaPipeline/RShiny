plots_phase_server <- function(input, output, session, globalDB) {
  ...
  output$UIStudyOneOrTwo <- renderUI({
    if(globalDB$studytype == "One"){
      output <- tagList()
      output[[1]] <- p("Phase difference plot not applicable to one-grouo study. ")
  ...
  output$UIStudyTwo <- renderUI({
    if((!is.null(UI_server_com$RVplotType2))&(UI_server_com$choose_StudyTwo)){
      ...
      if(globalDB$studytype=="Two"&UI_server_com$RVplotType2 == "circos_diff"){
        ...
        output[[4]] <- numericInput(ns("diffTimeStart"), "Starting Time: ", -6)
        output[[5]] <- numericInput(ns("diffTimeBandWidth"), "Band width around 0: ", 4)
        output[[6]] <- numericInput(ns("diffY"), "Set clock grid jump: ", 4)
        output[[7]] <- numericInput(ns("diffX"), "Set peak difference grid jump: ", 4)
        output[[8]] <- numericInput(ns("diffTextSize"), "Set tick size: ", 12)
        output[[9]] <- colourpicker::colourInput(ns("diffColorBand"), "Select color for the band around 0", "darkgreen")
        output[[10]] <- colourpicker::colourInput(ns("diffColorCon"), "Select color for the peak difference terminals", "grey80")
        output[[11]] <- colourpicker::colourInput(ns("diffColor0"), "Select color for 0 peak difference reference", "blue")
        output[[12]] <- selectInput(ns("diffLegendPosition"), "Set legend position",
                                    c("right" = "right",
                                      "left" = "left",
                                      "top" = "top",
                                      "bottom" = "bottom",
        ...
        output[[13]] <- actionButton(ns('plotDiff'), 'Plot', icon=icon("play"), class="btn-success")
        output
  ...
  observeEvent(input$plotLinked, {
    if(is.null(input$nTOJR_rows_selected)){
      ...
      TOJR.sel <- globalDB$TOJR[[input$nTOJR_rows_selected]]
      a.plot$p = DiffCircaPipeline::DCP_PlotPeakLink(x = CP.obj, TOJR = readRDS(paste0(globalDB$working.dir,  "/save/TOJR_", TOJR.sel$method, "_", TOJR.sel$cutoffType, "_", TOJR.sel$cutoffValue, ".rds"))$TOJR,
                                                    dPhase = ifelse2(db$dPhase, res.DRP, NULL),
  ...
  observeEvent(input$plotDiff, {
    if(is.null(input$nTOJR_rows_selected)){
      ...
      print("DEBUG in 1")
      TOJR.sel <- globalDB$TOJR[[input$nTOJR_rows_selected]]
      a.TOJR <- readRDS(paste0(globalDB$working.dir,  "/save/TOJR_", TOJR.sel$method, "_", TOJR.sel$cutoffType, "_", TOJR.sel$cutoffValue, ".rds"))$TOJR
      ...
      a.dPhase <- ifelse2(globalDB$dPhase, res.DRP, NULL)
      ...
      a.plot$p = DiffCircaPipeline::DCP_PlotPeakDiff(x = CP.obj,
                                                    ...
                                                    color.diff.connectline = input$diffColorCon,
