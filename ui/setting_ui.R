setting_ui <- function(id, label = "global setting"){
  ...
  tabPanel("Welcome", value=id,
           ...
           fluidRow(
             ...
             HTML('<p><width="1200">The DiffCircaPipeline Rshiny contains four panels:  <ul><li>Data input and output: input data and time points and set output directory. </li><li>Joint Rhythmicity: (flowchart step a) Categorize genes to four joint rhythmicity types. This step prepares gene sets for biological meaningful differential rhythmicity (DR) tests since DR parameter is only meaningful for genes rhythmic in both groups (RhyBoth) and DR fitness is only meaningful for genes rhythmic in at least one group. </li><li>Differential rhythmicity tests: (flowchart step b) perform DR fitness and/or DR parameter test to selected gene sets from step a. </li><li>Visualization: makes scatter plots, heatmaps, or circos peak plots. </li></ul> </p>'),
